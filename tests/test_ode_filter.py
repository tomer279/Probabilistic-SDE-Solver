"""
Tests for the Gaussian ODE filter (EKF0) and prior model.

Covers: one step on simple ODE, posterior contraction as step size decreases.
"""

import jax.numpy as jnp
from prob_sde.ode_filter import ode_filter_step, ode_integrator_factory
...
def test_ode_filter_step_returns_mean_cov():
    ...
    mean_new, cov_new = ode_filter_step(mean, cov, vector_field, (0.0, 0.1), prior)
    assert mean_new.shape == (2,)
    assert cov_new.shape == (2, 2)


def test_ode_filter_on_exponential_decay():
    """On dx/dt = -x, filter should move toward correct derivative."""
    prior = IWP2Prior(0.5)
...
def test_ode_integrator_step_signature():
    ...
    x0 = jnp.array(1.0)
    mean0 = prior.initial_mean(x0, vector_field(x0, 0.0))
